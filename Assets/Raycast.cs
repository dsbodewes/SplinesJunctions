...
public class Raycast : MonoBehaviour
{
    void FixedUpdate()
    {
        ...
        if ((Physics.Raycast(new Vector3(transform.position.x, transform.position.y + 2, transform.position.z), transform.TransformDirection(Vector3.forward), out hit, 15)) && hit.transform.tag == "Car")
        {
        ...
        else
        {
            ...
            Debug.DrawRay(new Vector3(transform.position.x, transform.position.y + 2, transform.position.z), transform.TransformDirection(Vector3.forward) * 15, Color.green);
            Stop = false;
        }
